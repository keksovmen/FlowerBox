# Описание
Плата считывает значения АЦП (ADC) с большего количества сигналов через матрицу, при изменении значений сообщает свое состояние на удаленный сервер через HTTPS. Либо же через MQTT.\
...
# MQTT
У каждого девайса есть **ID**, которое задается через приложение, либо при прошивке, **по умолчанию 0**.

## Точки куда отправляет свои данные:
- /cipher_box/**ID**/state - отправляет данные в случае изменения одного или нескольких входов на плате. Формат данных массив чисел,
	количество чисел зависит от настроек платы, от 8-24. Данные в виде: [int, int, ...]
