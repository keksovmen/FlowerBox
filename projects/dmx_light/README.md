...
- /dmx/**ID**/func - используется для настройки данных в пакете DMX, в теле json:\
		{"data":[\
			{\
				"i" : [0; 512],\
				"v" : [-1; 255],\
		...
		, где: "i" - индекс канала,\
		"v" - значение канала, если оно **присутствует и не равно -1**, то канал будет **статичным**, те всегда показывать это значение, если его не добавлять, то канал будет **динамическим**\
		"a" - точка начала движения,\
		"b" - точка окончания движения,\
		"T" - время в миллисекундах для движения,\
		"cycle" - флаг определяющий надо ли идти обратно.\
		**НЕ СТОИТ ОТПРАВЛЯТЬ БОЛЬШЕ ЧЕМ 40 КАНАЛОВ ЗА РАЗ, В ПАМЯТЬ МОЖЕТ НЕ ВЛЕЗТЬ И ВСЕ УМРЕТ**
...
- /dmx/**ID**/scenery - используется для создания анимаций, внутри JSON [подробнее тут](animation.md)
- /dmx/**ID**/clear - используется для отмены анимаций и изменения всех каналов в 0, внутри что угодно в идеале пустота, вообще не важно
