...
### `/input_matrix/{id}/state`

Текущее состояние всех кнопок, светит подсветка или нет, переведен ли тумблер.
...
Поле `state` содержит массив состояний всех 30 штук кнопок.


Сообщение отправляются каждый раз при изменении состояния кнопки. Даже при изменении через публикацию MQTT. Имеет флаг retain.
