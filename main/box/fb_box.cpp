...
std::string Box::toJson() const
{
	...
	std::for_each(_properties.begin(), _properties.end(), [&propIds](auto p){propIds.push_back(p.getId());});
	cJSON_AddItemToObject(obj, "properties_ids", cJSON_CreateIntArray(propIds.data(), _properties.size()));
