#pragma once



#include <stdint.h>

#include "freertos/FreeRTOS.h"
#include "freertos/queue.h"
#include "driver/rmt_tx.h"
#include "driver/rmt_rx.h"



#define OW_TX_MEM_BLOCK_SYMBOLS 64
#define OW_TX_QUEUE_DEPTH 4
#define OW_MAX_READ_BITS 64
#define OW_RX_MEM_BLOCK_SYMBOLS (OW_MAX_READ_BITS + 2)
#define OW_RX_MIN_NS 1000
#define OW_RMT_TIMEOUT_MS 1000
#define OW_RX_MARGIN_US 3



#ifdef __cplusplus
extern "C"
{
#endif



	typedef struct
	{
		rmt_channel_handle_t tx_channel;
		rmt_channel_handle_t rx_channel;
		rmt_encoder_handle_t copy_encoder;
		rmt_encoder_handle_t bytes_encoder;
		rmt_symbol_word_t *rx_buffer;
		size_t rx_buflen;
		QueueHandle_t rx_queue;
	} OW_t;



	bool ow_init(OW_t *ow, int gpio);
	void ow_deinit(OW_t *ow);

	void ow_send(OW_t *ow, uint32_t data);
	void ow_send_bit(OW_t *ow, uint32_t data);

	uint8_t ow_read(OW_t *ow);
	bool ow_read_bit(OW_t *ow);

	bool ow_reset(OW_t *ow);
	int ow_romsearch(OW_t *ow, uint64_t *romcodes, int maxdevs, uint32_t command);



#ifdef __cplusplus
}
#endif